...
// with stable IDs. The concrete MOT engine (ByteTrack / BoT-SORT / OC-SORT,
// selected by TrackerCfg) is hidden behind a pimpl so its dependency (motcpp,
// AGPL) stays isolated to a single .cpp — swapping the engine touches nothing else.
class MotTracker {
public:
    // Config-driven: selects the tracker (ByteTrack / BoT-SORT / OC-SORT) and its
    // association tuning. For
